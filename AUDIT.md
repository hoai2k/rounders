...
| # | Finding | Resolution |
|---|---|---|
| C1 | **CRITICAL** — Explosive bullets double-dipped: the direct-hit victim also took the full splash, making Popcorn Payload (~+50% effective damage for a −10% "cost") stronger than rare damage cards. | ✅ `explodeBullet` skipped players the bullet already hit; splash became purely area denial. **Revised 2026-08-17 (C1b)** — full immunity went too far: in a duel there is nobody else for splash to catch, so legendary Supernova measured as *just* +30% damage (36 → 48 a hit) and played as "the card did nothing". The direct victim now takes a **share** of the blast, scaling with its size (`coreShare = 0.25 + 0.12 × explosive`, capped 0.6) and without a second knockback. Measured point-blank: Popcorn Payload 41 (1.14×, still a 3-hit kill — C1's intent holds), Supernova 73 (2.03×). |
| C1c | Rarity inversion found while re-measuring C1b: epic Party Favor (76) out-damaged legendary Supernova (65) point-blank, because its shards spawn on top of the victim and double-dip where splash no longer can. | ✅ Supernova raised to `explosive += 2.8` so the legendary tops the family (73 vs 76 — peers, inversion gone). 🔭 The underlying shard double-dip is untouched and still makes point-blank Shrapnel Burst/Party Favor spike; a full explosive+shard balance pass is deferred. |
...
| L11 | **Tease gaps** — 17 places where a platform stopped 12–46px short of a wall, a tower, or another platform: a slot that *looks* like a fighter can slip through but is narrower than the 54px body. Worst offenders: Bonepit's towers 30px off both screen edges, Koi Temple's docks 40px off the torii pillars, Neon Grid's conveyor lanes 30px off the firewall panels, Static Circus's trapezes hung 20px apart. Plus two vertical squeezes (Cogwork's wall panels 38px above the side ledges, Rustyard's chassis 14px over a tier). | ✅ Every one widened past a body width (or sealed flush where a doorway made no sense: Sirocco's plank bridge now meets its mesa walls and spans the middle, Tidal Wreck's rigging plank meets the mast). New permanent tool: **`npm run audit-arenas`** scans every arena for horizontal doorways *and* vertical squeezes between 12px and a body width, reads the body size from `js/gameplay.js` so they can't drift apart, skips gaps occluded by a third solid, and exits non-zero on any finding. All 25 arenas scan clean. |
| L12 | Water was float-only: jump did nothing submerged, so leaving a pool meant drifting to the edge while it chewed your health. | ✅ **Swimming**: underwater, jump is a stroke — a repeatable paddle kick (no air-jump cost, steerable with left/right) that works you up to the surface, with a splash and a bob per stroke; one last kick breaches you out. Wall-kicks still take priority beside a pool wall, and jumping off a submerged floor is still a full jump. |
